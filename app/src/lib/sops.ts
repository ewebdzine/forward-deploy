...
/** First paragraph under "## What this covers", for index lines. */
export function extractSummary(content: string): string {
  const match = content.match(
    /##\s*What this covers\s*\r?\n+([^\r\n#][^\r\n]*)/i
  );
  return (match?.[1] ?? "").trim();
}

/**
 * The department's INDEX.md - one summary line per SOP, the Canonify-style
 * breadth map. Regenerated by the app on every SOP commit; never hand-edited.
 */
export function buildIndexMarkdown(
  ...
  const lines = sops
    ...
    .map((s) => {
      ...
      const summary = extractSummary(s.content);
      return `- **${s.topic}** (\`${s.slug}.md\`)${tools}${summary ? ` - ${summary}` : ""}`;
    });
  return `# ${departmentName} - SOP index

One line per SOP in this folder. Regenerated automatically by Forward Deploy on every SOP
commit - do not edit by hand. The plan builder and the Claude Code gates route through this
file the way Canonify routes through \`CANONIFY.md\`.
  ...
  try {
    ...
    for (const dept of departments.filter((d) => d !== departmentSlug)) {
      for (const sop of await listSops(dept)) {
        const summary = extractSummary(sop.content);
        otherLines.push(
