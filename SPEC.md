...
| **Billed-cost reconciliation (Anthropic Admin API)** | optional `ANTHROPIC_ADMIN_KEY`: pull the org Usage & Cost report (daily, per-key/workspace, actual billed dollars) and show "billed vs estimated" on an admin usage panel - true invoice truth beside the per-user/per-plan token estimates, which remain the only way to attribute cost to people and plans. Also auto-flags stale PRICE_PER_MTOK_* env (estimate drifting from billed) |
...
| **Two-role doctrine + department developers** | roles collapsed to **developer** (source-control side; administers the app) and **user** (documents SOPs, builds plans); legacy admin/manager enum values tolerated and migrated. Departments optionally assign a developer; with exactly one developer in the company they're the effective developer everywhere automatically; unassigned + multiple developers = nudges fan out to all. Plan submissions nudge the department's developer; SOP commits mentioning undocumented software nudge them with the exact capture-software command. **Auto-researching new software via cron was deliberately rejected**: capture-software ends with human review before canons commit; unattended research would commit unreviewed vendor docs that silently ground future plans |
| **Docs-live-with-code doctrine (split repo deliberately rejected)** | canons, SOPs, and software docs stay in the code repo. Rationale: half a megabyte of markdown is invisible next to any codebase; app commits are prefix-filterable (`SOP:`, `Software canon:`) and never conflict with code branches; and three core behaviors depend on same-repo - Canonify staleness (`git log <verified>..HEAD -- <cited file>` can't cross repos), the plan builder's single breadth map, and automatic context for any dev running Claude Code in the repo. Escape hatch for objectors: `git sparse-checkout` excludes docs/ per working copy (at the cost of Claude context) |
...
| **Email digest (Gmail/Outlook)** | the email sibling of the Slack digest: weekly cron per opted-in user reads mail threads **between Forward Deploy users only** (roster-matched addresses; external mail categorically untouched) with a **mutual-consent rule** - a thread is mined only if every FD participant opted in. Claude summarizes, cross-references the SOP corpus, and writes both "needs documenting" gaps AND **staleness signals** (email describes a process differently than its committed SOP - suggest an update) to the same insights/captures pipeline with pre-seeded builders. Mechanics: per-user OAuth is unavoidable here (gmail.readonly / Graph Mail.Read) - the one legitimate OAuth exception to the credentials doctrine, since mailboxes have no paste-a-key equivalent; per-company app registration (Google Workspace *internal* apps skip restricted-scope verification); encrypted refresh tokens in DB (settings-in-DB prerequisite); summaries stored, never raw mail |
