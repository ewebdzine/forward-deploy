...
| **Slack weekly digest -> documentation worklist** | optional Slack app (bot token env, absent = zero residue) + weekly cron: for opted-in Forward Deploy users only (email-matched), summarize their week from bot-invited channels (public only, NEVER DMs; store summaries, never raw logs) via Claude, then cross-reference discussed processes/tools against the SOP corpus to produce a per-person "needs documenting" list in the `insights` table. Dashboard surfaces it; each gap is a click that opens the SOP builder pre-seeded with the Slack summary - kills the blank-page problem and makes SOP coverage self-expanding. This is the original cross-department insights sweep with Slack as its richest source |
