...
| **OAuth connect flows (post-wizard)** | replace pasted tokens with "Connect" buttons where providers allow self-serve app creation: GitHub via the **app manifest flow** (wizard creates a GitHub App under the company's own account, install screen picks repos, installation tokens - the gold path), Slack via its manifest flow, Bitbucket via a guided OAuth-consumer setup (two-field form, then the authorize dance; app stores + refreshes expiring tokens). Anthropic and SMTP stay paste-a-key (no OAuth exists). Requires the settings-in-DB architecture (rotating tokens can't live in env). Deliberately NO central shared OAuth app - per-company apps keep the no-vendor-in-the-loop trust story true and callback URLs sane on self-hosted domains |
